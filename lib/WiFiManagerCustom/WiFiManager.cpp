...
void WiFiManager::setupConfigPortal() {
  ...
  server->on("/customsettings", std::bind(&WiFiManager::handleCustomSettings, this));
  server->on("/customsettingssave", std::bind(&WiFiManager::handleCustomSettingsSave, this));
...
void WiFiManager::handleRoot() {
  ...
  page += _apName;
  page += "</h1>";
  page += F("<h3>Connection Manager</h3>");
...
/*  GET SOME FILE FROM EPROM IF IT EXIST AND PASS IT TO A BROWSER!!! */
...
/** Handle the custom settings page, which is actually json editor */
void WiFiManager::handleCustomSettings() {
  DEBUG_WM(F("Custom Settings JSON editor page!"));
  ...
  String page = FPSTR(HTTP_HEAD);
  page.replace("{v}", "Info");
  page += FPSTR(HTTP_INCLUDE_SCRIPTS);
  page += FPSTR(COLOR_PICKER_INCLUDE_SCRIPTS);
  page += FPSTR(HTTP_HEAD_END);
  page += FPSTR(MQTT_HTTP_STYLE);
  page += FPSTR(MQTT_SETTINGS_FORM);
  page += FPSTR(NEOPIXEL_SETTINGS_FORM);
  page += FPSTR(MQTT_SETTINGS_JS);
  page += FPSTR(HTTP_END);

  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);

    DEBUG_WM(F("Custom Settings JSON editor page SENT"));
}

/** this will actually get/upload the json to a ESP8266 */
void WiFiManager::handleCustomSettingsSave() {
    ...
    DynamicJsonBuffer newSettingsJB;
...
boolean WiFiManager::fileExists(String path) {
  SPIFFS.begin();
  ...
  if (SPIFFS.exists(path + ".gz")) {
    WiFiManager::getFileFromEprom(path + ".gz");
    return true;
  } else if (SPIFFS.exists(path)) {
    WiFiManager::getFileFromEprom(path);
    return true;
  }
...
void WiFiManager::getFileFromEprom(String path) {
  File file = SPIFFS.open(path, "r");
  size_t sent = server->streamFile(file, WiFiManager::getContentType(path));
  file.close();
  Serial.println(String("\tSent file: ") + path);
}

void WiFiManager::uploadFilesToEprom() {
    File fsUploadFile;
...
void WiFiManager::handleNotFound() {
  ...
  if (WiFiManager::fileExists(server->uri())){
    return;
  }
